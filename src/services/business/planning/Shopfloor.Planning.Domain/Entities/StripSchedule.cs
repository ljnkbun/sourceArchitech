using Shopfloor.Core.Models.Entities;

namespace Shopfloor.Planning.Domain.Entities
{
    public class StripSchedule : BaseEntity
    {
        public StripSchedule()
        {
            StripScheduleDetails = new HashSet<StripScheduleDetail>();
            StripSchedulePlanningDetails = new HashSet<StripSchedulePlanningDetail>();
            StripFactorySchedules = new HashSet<StripFactorySchedule>();
            FPPOs = new HashSet<FPPO>();
        }
        public int? LineId { get; set; }
        public int? MachineId { get; set; }
        public decimal Quantity { get; set; }
        public decimal? ProfileEfficiencyValue { get; set; }
        public decimal? OrderEfficiencyValue { get; set; }
        public decimal? StripEfficiencyValue { get; set; }
        public int? LearningCurveEfficiencyId { get; set; }
        public string Gauge { get; set; }
        public DateTime FromDate { get; set; }
        public DateTime ToDate { get; set; }
        public int FromWorkingHours { get; set; }
        public int ToWorkingHours { get; set; }
        public string Description { get; set; }
        public bool IsManualPlanning { get; set; }
        public virtual ICollection<StripScheduleDetail> StripScheduleDetails { get; set; }
        public virtual ICollection<StripSchedulePlanningDetail> StripSchedulePlanningDetails { get; set; }
        public virtual ICollection<StripFactorySchedule> StripFactorySchedules { get; set; }
        public virtual ICollection<FPPO> FPPOs { get; set; }
    }
}
