...
namespace Shopfloor.IED.Application.Validations.SewingOperationLibs
{
    public class UpdateSewingOperationLibCommandValidator : AbstractValidator<UpdateSewingOperationLibCommand>
    {
        ...
        public UpdateSewingOperationLibCommandValidator(ISewingTaskLibRepository repository)
        {
        ...
        private bool IsLineNumberUnique(UpdateSewingOperationLibCommand command)
        {
            return command?.SewingOperationLibBOLs.DistinctBy(m => m.LineNumber).Count() == command?.SewingOperationLibBOLs.Count;
        }
        private bool IsValid(UpdateSewingOperationLibCommand command)
        {
        ...
        private async Task<bool> IsLessThanTwoMachineTasksWithSameProfileAsync(UpdateSewingOperationLibCommand command, CancellationToken token)
        {
