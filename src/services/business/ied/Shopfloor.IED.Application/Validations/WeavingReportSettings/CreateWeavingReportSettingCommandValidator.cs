...
{
    public class CreateWeavingReportSettingCommandValidator : AbstractValidator<CreateWeavingReportSettingCommand>
    {
        private readonly IWeavingIEDRepository _weavingIed;
        private readonly IWeavingReportSettingRepository _reportSetting;

        public CreateWeavingReportSettingCommandValidator(IWeavingIEDRepository weavingIed, IWeavingReportSettingRepository reportSetting)
        {
            ...
            RuleFor(p => p.WeavingIEDId)
                .MustAsync(IsExistReportSettingAsync)
                .WithMessage("Report Setting was found.");
            ...
            RuleForEach(p => p.WeavingReportSettingDetails).ChildRules(childWeavingReportSettingDetails =>
            {
                ...
                childWeavingReportSettingDetails.RuleFor(p => p.Repeat)
                    .GreaterThan(0).WithMessage("{PropertyName} must greater than 0.");

                childWeavingReportSettingDetails.RuleFor(p => p.GroupIndex)
                    .GreaterThan(0).WithMessage("{PropertyName} must greater than 0.");
        ...
        private async Task<bool> IsExistWeavingIEDAsync(int weavingIEDId, CancellationToken token) => await _weavingIed.IsExistAsync(weavingIEDId);      
        
        private async Task<bool> IsExistReportSettingAsync(int weavingIEDId, CancellationToken token) => !await _reportSetting.IsExistByWeavingIEDAsync(weavingIEDId);

        private void IsCheckReportSettingDetail(CreateWeavingReportSettingCommand data,
            ValidationContext<CreateWeavingReportSettingCommand> context)
        {
