...
namespace Shopfloor.IED.Application.Validations.SewingFeatureLibs
{
    ...
    {
        public CreateSewingFeatureLibCommandValidator()
        {
            RuleFor(p => p.Name)
        ...
        private bool IsLineNumberUnique(CreateSewingFeatureLibCommand command)
        {
            return command?.SewingFeatureLibBOLs.DistinctBy(m => m.LineNumber).Count() == command?.SewingFeatureLibBOLs.Count;
        }
        private bool IsValid(CreateSewingFeatureLibCommand command)
        {
