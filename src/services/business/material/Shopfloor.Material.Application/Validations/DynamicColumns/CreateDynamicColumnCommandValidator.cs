...
{
    public class CreateDynamicColumnCommandValidator : AbstractValidator<CreateDynamicColumnCommand>
    {
        private readonly IDynamicColumnRepository _dynamicColumnRepository;

        public CreateDynamicColumnCommandValidator(IDynamicColumnRepository dynamicColumnRepository)
        {
        ...
        private async Task<bool> IsUniqueAsync(CreateDynamicColumnCommand command, CancellationToken cancellationToken)
        {
            return await _dynamicColumnRepository.IsUniqueAsync(command.Code, command.CategoryCode);
        }
    }
}