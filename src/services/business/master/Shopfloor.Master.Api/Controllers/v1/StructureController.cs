...
namespace Shopfloor.Master.Api.Controllers.v1
{
    ...
    public class StructureController : BaseApiController
    {
        ...
        [HttpGet]
        public async Task<IActionResult> Get([FromQuery] StructureParameter filter)
        {
            return Ok(await Mediator.Send(new GetStructuresQuery()
            {
        ...
        {
            return Ok(await Mediator.Send(new GetStructureQuery { Id = id }));
        }
        ...
        {
            return Ok(await Mediator.Send(new DeleteStructureCommand { Id = id }));
        }
    }
}
