...
{
    public class UpdateProblemTimelineCommand : IRequest<Response<int>>
    {
    ...
    public class UpdateProblemTimelineCommandHandler : IRequestHandler<UpdateProblemTimelineCommand, Response<int>>
    {
        private readonly IProblemTimelineRepository _repository;
        public UpdateProblemTimelineCommandHandler(IProblemTimelineRepository repository)
        {
            _repository = repository;
        }
        public async Task<Response<int>> Handle(UpdateProblemTimelineCommand command, CancellationToken cancellationToken)
        {
            var entity = await _repository.GetByIdAsync(command.Id);

            if (entity == null) return new ($"ProblemTimeline Not Found.");
